...
export function StoreCoupons({ storeName }: StoreCouponsProps) {
  // Convert store name to lowercase for matching with our data
  const storeSlug = storeName.toLowerCase().replace(/\s+/g, "")

  // Get coupons for this specific store
  const storeCoupons = allCoupons[storeSlug as keyof typeof allCoupons] || []

  const [couponsState, setCouponsState] = useState(storeCoupons)
  const [selectedCoupon, setSelectedCoupon] = useState<(typeof storeCoupons)[0] | null>(null)
  ...
  const openCouponModal = (coupon: (typeof storeCoupons)[0]) => {
    ...
    <>
      ...
      {couponsState.length === 0 ? (
        <div className="text-center py-12">
          <h3 className="text-xl font-semibold mb-2">No coupons found for {storeName}</h3>
